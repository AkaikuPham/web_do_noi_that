...
class CheckoutController extends Controller
{
    ...
    public function delivery()
    {
        ...
        foreach ($carts as $cart) {
            $prDetailId = $cart->options['product_detail_id'];
            $productDetail = $this->productDetailRepository->findById($prDetailId);
            if (!empty($productDetail)) {
                $orderDetails = $this->orderDetailRepository->getOrderDetails($productDetail->id);
                ...
                if ($cart->qty > $quantity) {
                    ...
                    return redirect()->back()->with([
                        'error' => "Số lượng trong kho của sản phẩm $name còn $quantity"
                    ]);
                }
            }
        }
